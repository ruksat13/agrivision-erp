...
| `safetySource` | string \| null | ✔︎ | **Where the seven above came from** — "Container label, photographed 2026-08-16", or "PLACEHOLDER — demonstration data". Printed on the invoice panel verbatim. **Required whenever any of the seven above carries a value**, and refused in three places: the Product page dialog, `setSafetyData()`/`createProduct()`, and `firestore.rules` on the `products` write itself — the last is the control (`CLAUDE.md` §6). `phiDays: 0` counts as a value; `approvedCropsBn: []` does not. A blank string is an absence to the service, which stores it as `null`, and a value to the rules, which cannot normalise and so refuse it. All eight null together stays legal — that is how safety data is cleared. Added while building Feature 3: §9 of this document is entirely about the provenance of these numbers, and a panel that prints them without saying where they came from is the failure §9 describes |
...
| **`purchases`** ✅ | `purchaseNo` ✔︎ (`AINP-2026-08-0000013`, from a month counter, mirroring `sales.invoiceNo`), `supplierId` ✔︎, `supplierName`/`supplierCode` *(denorm, historical)*, `invoiceNo` (the **supplier's** invoice number, free text — theirs, not ours), `purchaseDate` ✔︎, `dueDate`, `officeId` ✔︎, `subTotal`, `shipping`, `additional`, `discount`, `grandTotal`, `status` (`Stock Done`\|`Cancelled`), `note`, `ruleChecks[]`, `createdBy`. **Document ID is the purchase number** | `Purchase.js:33` |
| **`purchase_items`** ✅ | `purchaseId` ✔︎, `purchaseNo`, `lineNo`, `productId` ✔︎, `productName`/`productCode`/`packSize`/`cartonQty` *(denorm, historical)*, `qty` ✔︎, `unitPrice` ✔︎, `lineTotal`, `purchaseDate`, `supplierId`, *(`lotNo`, `mfgDate`, `expiryDate` reserved for Feature 4)*. Top-level, for D2's reasons | `Purchase.js:89` — which rendered **one hardcoded line** on every purchase invoice, §4.3 |
| | A purchase writes its `purchase` stock movements and raises the supplier's payable **in the same batch as the invoice**, exactly as `createSale()` does in the other direction. It also runs the banned-product rule: `INTERNAL-PLAN.md` §3 requires the Feature 2 check on the purchase path too, so banned stock cannot be received. Unlike a sale that block is **not** overridable | |
| **`purchase_returns`** ✅ | `returnNo` ✔︎ (`AIPR-R-000001`), `purchaseId` → `purchases`, `purchaseNo`, `supplierId` ✔︎, `supplierName` *(denorm)*, `returnDate` ✔︎, `officeId` ✔︎, `items[{productId, name, qty, unitPrice, lineTotal}]`, `amount`, `reason` ✔︎, `status` (`Pending`\|`Approved`\|`Cancelled`), `note`. Auto-ID | `PurchaseReturn.js`. Writes `purchase_return` movements **on approval**, and reduces the payable by the same amount |
| **`boms`** ✅ | `bomNo` ✔︎ (`AIB-000182`), `productId` ✔︎ (the product the recipe **makes**), `productName` *(denorm)*, `items[{productId, name` *(denorm)*`, ratio}]` ✔︎ — `ratio` is **how much of that material one unit of output consumes**, so a 50 ml bottle filled from a 200 ltr drum has `ratio: 0.05` and its bottle and label have `ratio: 1`, `effectiveFrom`, `status`. **Document ID is the BOM number** | `Batch.js:8` — **renamed from "Batch"**, decision 2 in `SCREEN-AUDIT.md` §7 |
| **`repackings`** ✅ | `repackNo` ✔︎ (`AIR-006391`), `bomId` ✔︎ → `boms`, `outputProductId` ✔︎ *(denorm from the BOM)*, `qty` ✔︎ (output units), `cartonQty`, `officeId` ✔︎, `repackDate`, `consumed[{productId, name, ratio, qty}]` (the recipe multiplied out, **snapshotted** so the run still reads correctly after the BOM is edited), `status` (`Pending`\|`Done`\|`Cancelled`), `note` | `Repacking.js:21` |
| | A repack **consumes and produces in one batch**: one `repack_out` per material at `ratio × qty`, and one `repack_in` for `qty` of the output product. It is the only place in the system where stock moves in both directions at once, and the reason a BOM is worth having at all. Refused if any material is short — the alternative is a negative balance, which `verify.mjs` asserts against | |
...
| **`expenses`** ✅ | `code` ✔︎ (`AIEX-012514`, from the `expenses` counter), `voucherNo` (`V:218`, from the `expense_vouchers` counter), `type` ✔︎ (`Expense`\|`Income` — the screen posts both and the badge says which), `headId` ✔︎ → `expense_heads`, `headName` *(denorm)*, `receivedById` → `users`, `receivedByName` *(denorm)*, `amount` ✔︎, `date` ✔︎, `status` (`Pending`\|`Approved`\|`Cancelled`), `note`. **Document ID is the code** | `Expense.js` |
| | `headName` is denormalised so the list needs no join, and is **rewritten** when the head is renamed rather than being a historical record (§2 rule 1) — an expense head is the same head after a spelling fix, unlike a customer name on an old invoice | |
