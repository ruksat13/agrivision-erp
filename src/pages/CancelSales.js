...
function CancelSales() {
    const [activeStatus, setActiveStatus] = useState(ROW_STATUS);
    const [viewRow, setViewRow] = useState(null);
    const [ordersOpen, setOrdersOpen] = useState(true);

    // Both derived from CANCEL_ROWS, so a badge cannot claim rows the table
    // below does not show.
    const rowsFor = (status) => (status === ROW_STATUS ? CANCEL_ROWS : []);
    const statusCounts = Object.fromEntries(salesStatuses.map(s => [s, rowsFor(s).length]));
    const visibleRows = rowsFor(activeStatus);
